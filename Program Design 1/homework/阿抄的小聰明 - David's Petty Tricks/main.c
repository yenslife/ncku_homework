...
#include <stdbool.h>
void sort_pointers(int **ptr_arr, int *arr, int size) {
    int maxNum = 0, *maxPointer;
    bool check[101] = {};
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (arr[j] >= maxNum && check[j] != true) {
                maxNum = arr[j];
                maxPointer = &arr[j];
            }
        }
        for (int j = 0; j < size; j++) if (maxPointer == &arr[j] && check[j] != true) check[j] = true;
        ptr_arr[i] = maxPointer;
        //printf("maxIndex = %d\n", *maxIndex);
        maxNum = 0;
    }
    //printf("a = %d, b = %d\n", (uintptr_t)&arr[0], (uintptr_t)ptr_arr[0]);
    for (int i = 0; i < size; i++) printf("%d ", **(ptr_arr + i));
}