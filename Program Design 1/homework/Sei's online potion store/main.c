...
int cmp(const void *a, const void *b) {
    order_t *timeA = (order_t *)a;
    order_t *timeB = (order_t *)b;
    if (timeA->pick_up_time.hour * 60 + timeA->pick_up_time.minute - timeB->pick_up_time.hour * 60 - timeB->pick_up_time.minute > 0) return 1;
    else if (timeA->pick_up_time.hour * 60 + timeA->pick_up_time.minute - timeB->pick_up_time.hour * 60 - timeB->pick_up_time.minute == 0) return 0;
    else if (timeA->pick_up_time.hour * 60 + timeA->pick_up_time.minute - timeB->pick_up_time.hour * 60 - timeB->pick_up_time.minute < 0) return -1;
}
        ...
        for (int j = 0; j < orders[i].item_cnt; j++) {
            for (int k = 0; k < avaliable_item_cnt; k++) {
                if (strcmp(orders[i].items[j].item_name, avaliable_items[k].item_name) == 0) {
                    orders[i].total_cost += orders[i].items[j].cnt * avaliable_items[k].cost;
                }
            }
        }
    }
